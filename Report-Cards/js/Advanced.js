...
// ═══════════════════════════════════════════════════════════
// GLOBAL HELPERS — image loading, grade extraction
// Used by all generators and advanced feature functions
// ═══════════════════════════════════════════════════════════
...
// ─── 2. PARENT ACKNOWLEDGEMENT LINE ──────────────────────────────────────
function drawParentAcknowledgement(doc, yPos) {
    const pageWidth = doc.internal.pageSize.width;
    doc.setFontSize(8);
    doc.setFont(undefined, 'bold');
    doc.setTextColor(0, 0, 0);
    doc.text("Parent/Guardian Acknowledgement:", 20, yPos);
    doc.setFont(undefined, 'normal');
    doc.line(75, yPos, pageWidth - 20, yPos);
    doc.setFontSize(7);
    doc.setTextColor(100, 100, 100);
    doc.text("Signature: ____________________   Date: ________________   Contact: ____________________", 20, yPos + 5);
}

// ─── SHARED: draw remarks + parent ack + signatures + QR safely ──────────
// Handles overflow: if remarks push too close to bottom, starts a new page
// doc_or_combined: the jsPDF instance (doc or combinedDoc)
// yPos: where remarks section starts
// student, stats, pageWidth, pageHeight all passed in
async function drawBottomSection(d, yPos, student, pageWidth, pageHeight) {
    const BOTTOM_SAFE   = pageHeight - 50;  // minimum safe Y before footer zone
    const FOOTER_TOP    = pageHeight - 42;  // parent ack starts here
    const SIG_Y         = pageHeight - 22;
    ...
    let remarkY = yPos + 6;
    remarkLines.forEach(line => { d.text(line, 20, remarkY); remarkY += 5; });
    // Two blank handwritten lines
    d.line(20, remarkY + 2, pageWidth - 20, remarkY + 2);
    d.line(20, remarkY + 8, pageWidth - 20, remarkY + 8);
    remarkY += 12;

    // If remarks have pushed into the footer zone — add a continuation page
    if (remarkY > BOTTOM_SAFE) {
        d.addPage();
        // Thin header strip on continuation page
    ...
    d.setFontSize(8); d.setFont(undefined,'bold'); d.setTextColor(0,0,0);
    ...
    d.line(20, SIG_Y + 5, 80, SIG_Y + 5);
    d.setFontSize(7); d.setFont(undefined,'normal');
    d.text('Signature & Date', 20, SIG_Y + 9);

    d.setFontSize(9); d.setFont(undefined,'bold');
    d.text('Head Teacher:', pageWidth / 2 + 20, SIG_Y);
    d.line(pageWidth/2 + 20, SIG_Y + 5, pageWidth/2 + 65, SIG_Y + 5);
    d.setFontSize(7); d.setFont(undefined,'normal');
    d.text('Signature & Stamp', pageWidth / 2 + 20, SIG_Y + 9);

    // Footer text
    d.setFontSize(7); d.setTextColor(128,128,128);
    d.text(`Generated: ${new Date().toLocaleDateString()}  |  KANYADET PRI & JUNIOR SCHOOL`,
        pageWidth / 2, FOOTER_TEXT_Y, { align: 'center' });

    // QR code — bottom-right corner, uses current last page
    await addQrCode(d, student, pageWidth - 26, SIG_Y - 18, 22);
}
...
async function generateStudentReportCard(student, includeWatermark = true) {
    ...
    // Remarks + parent acknowledgement + signatures + QR (overflow-safe)
    await drawBottomSection(doc, summaryBottom + 6, student, pageWidth, pageHeight);
...
async function generateBulkReportCards() {
    ...
    try {
        ...
        for (let i = 0; i < studentsToProcess.length; i++) {
            ...
            if (logoRes.loaded) {
                try { combinedDoc.addImage(logoRes.img, 'PNG', 15, 5, 25, 25); } catch(e) {}
            }
            combinedDoc.setTextColor(255, 255, 255);
            combinedDoc.setFontSize(18); combinedDoc.setFont(undefined, 'bold');
            ...
            combinedDoc.setFontSize(9); combinedDoc.setFont(undefined, 'normal');
            ...
            if (studentImageData.loaded) {
                try {
                    const imgW = 30, imgH = 35;
                    combinedDoc.addImage(studentImageData.img, 'JPEG', pageWidth - 20 - imgW, yPos + 2.5, imgW, imgH);
                } catch(e) {}
            }

            combinedDoc.setFontSize(10); combinedDoc.setFont(undefined, 'bold');
            combinedDoc.text('STUDENT INFORMATION', 20, yPos + 8);
            combinedDoc.setFont(undefined, 'normal'); combinedDoc.setFontSize(9);
            const studentInfo = [
                [`Name: ${student['Official Student Name'] || 'N/A'}`, `Assessment No: ${student['Assessment No'] || 'N/A'}`],
                [`UPI: ${student['UPI'] || 'N/A'}`, `Grade: ${student['Grade'] || 'N/A'}`],
                [`Gender: ${student['Gender'] || 'N/A'}`, `Class: ${student['Class'] || 'N/A'}`]
            ...
            // Performance table
            yPos = 95;
            combinedDoc.setFont(undefined, 'bold'); combinedDoc.setFontSize(11);
            ...
            // Remarks + parent ack + signatures + QR (overflow-safe)
            await drawBottomSection(combinedDoc, yPos + 6, student, pageWidth, pageHeight);
...
//  1. QR Code on report card
//  2. Parent acknowledgement line
//  3. Class league table PDF
//  4. Subject analysis report PDF
//  5. Missing data highlighter on report card
//  6. Duplicate student detector
//  7. Per-grade performance dashboard (modal)
//  8. Export to Excel (.xlsx)
//  9. Preview before print (modal)
//  10. Bulk WhatsApp/SMS share text
// ═══════════════════════════════════════════════════════════════════════════

// ─── 1. QR CODE HELPER ───────────────────────────────────────────────────
// Generates a simple URL-based QR via google charts API and embeds in PDF
...
// Already handled in getGradeFromScore returning 'Not Assessed' for NaN.
// Additional: highlight in the table display (called from renderTable in HTML side)
function highlightMissingScores(doc, student, tableStartY) {
    // Called after autoTable — walk subjects and flag any missing
    const subjects = getSubjects(student);
    const missing = subjects.filter(s => {
        const v = student[s];
        return v === null || v === undefined || v === '' || isNaN(parseFloat(v));
    });
    return missing; // caller shows warning
}
...
async function previewReportCard(studentId) {
    ...
    // Convert to blob URL and show in modal iframe
...
// Bulk WhatsApp — generate all in one go as text file
...
// Hook into existing addReportCardControls
...
// Also patch the per-row buttons by overriding renderTable rows to add Preview + WhatsApp icons
// This is done by hooking into the existing row render — we add buttons via event delegation
